...
class XiaomiMyBand:
    ...
    def publish(self):
        ...
        time.sleep(1)
        message = {}
        # mensajes json acelerometro          
        message['eje_x'] = self.simulate_x_position()
        message['eje_y'] = self.simulate_y_position()
        message['eje_z'] = self.simulate_z_position()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        ...
        channel.basic_publish(exchange='', routing_key='acelerometro', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión

        message = {}
        # mensajes json acelerometro          
        message['medicamento'] = self.simulate_medicines()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['rtime'] = self.simulate_time_minutes()
        message['producer'] = self.producer
        message['model'] = self.model
        message['hardware_version'] = self.hardware_version
        message['software_version'] = self.software_version
        ...
        channel.basic_publish(exchange='', routing_key='medicamento', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión
    ...
    def simulate_time_minutes(self):
        x = datetime.now() + timedelta(minutes = random.randint(0, 9) ) 
        return x.strftime("%d:%m:%Y:%H:%M:%S")

    def simulate_medicines(self):
        med = ['Paracetamol 500 mg','Ibuprofeno 750 mg','Insulina 13gr','Furosemida  2mg','Piroxicam 2 gm','Tolbutamida 6mg'] 
        return med [random.randint(0,5)]